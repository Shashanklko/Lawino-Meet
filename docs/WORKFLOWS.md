...
sequenceDiagram
    autonumber
    actor User as User (Client / Lawyer)
    ...
    AuthCtrl-->>ClientApp: 200 OK Token Response
...
sequenceDiagram
    ...
    DB-->>ChatSvc: Tokens Deducted and Wallet Updated
    ...
    Client->>React: Type and Send Message
...
sequenceDiagram
    ...
    ConsultSvc-->>React: Consultation Response (Phone: Masked, Email: Masked)

    note right of Lawyer: Step 2: Custom Fee Set and Approval
    Lawyer->>ConsultSvc: PUT /api/consultations/{id}/fee?fee=150.00
    ConsultSvc->>DB: Update customFee = 150.00 and Status = ACCEPTED
    
    note right of Client: Step 3: Checkout and Escrow Payment
    Client->>PaySvc: POST /api/payments/checkout (consultationId: 101)
    PaySvc->>DB: Process Payment and Calculate Split (80% Lawyer Wallet, 20% Platform Fee)
    PaySvc->>DB: Update Consultation status = CONFIRMED, isContactInfoDisclosed = TRUE
    PaySvc->>ConsultSvc: Generate Jitsi Video URL
    PaySvc-->>React: Payment Transaction Record and Unmasked Contact Info
```
...
sequenceDiagram
    ...
    Lawyer->>ConsultSvc: Accept and Provide Office Address
    ConsultSvc->>DB: Update status = ACCEPTED, lawyerOfficeAddress = Office Address
    
    Client->>PaySvc: POST /api/payments/checkout
    PaySvc->>DB: Calculate Offline Split (90% Lawyer Share, 10% Platform Fee)
    PaySvc->>DB: Update Consultation status = CONFIRMED, Disclose Address and Unmasked Contact
    PaySvc-->>React: Appointment Confirmed and Full Office Address Dispatched
```
...
sequenceDiagram
    ...
    participant PaySvc as Payment and Payout Service
    participant DB as MySQL Database
    ...
    Lawyer->>PaySvc: POST /api/payouts/request (Amount: $250, Bank Details)
    ...
    note over Admin, DB: 3. Admin Approval and Settlement
...
sequenceDiagram
    ...
    AdminSvc-->>Admin: Dispute Resolved and Audit Logged
```
